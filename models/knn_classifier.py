...
class KNNClassifier:
    ...
    def train(self, X_train, y_train, optimize=True, verbose=True):
        ...
        if optimize:
            ...
            grid_search = GridSearchCV(knn, param_grid, cv=cv, scoring='f1_macro', n_jobs=1, verbose=1 if verbose else 0)
            ...
            self.model = KNeighborsClassifier(n_neighbors=config.KNN_N_NEIGHBORS, weights=config.KNN_WEIGHTS, metric=config.KNN_METRIC)
    ...
    def save(self, path=None):
        if path is None:
            path = config.MODELS_DIR / "knn_model.joblib"
